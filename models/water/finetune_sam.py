...
def advanced_preprocessing_pipeline(img, target_size=(256, 256)):
...
def stack_preprocessing_variants(image_path, target_size=(256, 256)):
    """
    Loads an image and returns a 12-channel numpy array stacking:
    - original RGB
    - kaggle preprocessed RGB
    - github preprocessed RGB (use 'original' key)
    - advanced preprocessed RGB (use 'preproc' key)
    ...
    stacked = np.concatenate([
        arr_original, arr_kaggle, arr_github, arr_advanced
    ], axis=-1)
    return stacked

def patch_first_conv(model, new_in_channels=12):
    import torch.nn as nn
    old_conv = model.image_encoder.patch_embed.proj
    new_conv = nn.Conv2d(
...
# --- Select 200 base images ---
random.shuffle(paired)
paired_subset = paired[:200]
...
# --- HUGGINGFACE SAM FINE-TUNING TEMPLATE ---
# Load model and processor (resume from last checkpoint for faster convergence)
model = SamModel.from_pretrained('models/water/checkpoints/SAM-water-hf')
model = patch_first_conv(model, new_in_channels=12)
...
epochs = 1  # Only 1 epoch as requested
batch_size = 1
...
for epoch in range(epochs):
    ...
    for idx, (img_path, mask_path) in enumerate(tqdm(train_pairs, total=len(train_pairs))):
        stacked = stack_preprocessing_variants(img_path, target_size=(256, 256))
        mask_np = np.array(Image.open(mask_path).convert('L').resize((256, 256), Image.NEAREST))
        if np.sum(mask_np) == 0:
            continue
